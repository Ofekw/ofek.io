...
import footerHTML from 'raw!./footer.html';
...
// I think I understand async functions
...
async function writeTo(el, message, index, interval, mirrorToStyle, charsPerInterval){
  ...
  if (index < message.length) {
    ...
    if (comma.test(thisSlice)) thisInterval = interval * 15;
    if (endOfBlock.test(thisSlice)) thisInterval = interval * 25;
    if (endOfSentence.test(thisSlice)) thisInterval = interval * 35;
...
function getBrowserPrefix() {
  ...
  browserPrefix = getPrefix(); // could be empty string, which is ok
...
function getEls() {
  // We're cheating a bit on styles. (don't tell anyone)
